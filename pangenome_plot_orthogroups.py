...
while n <= 10: # Number of bootstraps
    ...
    for k, v in groups:
        ...
        v = v.reset_index()
        if number == 1:
            core_dict[number] = set(core) & set(v['Orthogroup'])
            pangenome = set(core) & set(v['Orthogroup'])
        else:
            core_dict[number] = set(core) & set(v['Orthogroup'])
            pangenome.update(set(core) ^ set(v['Orthogroup']))
...
plt.ylabel("Orthogroup Number", fontsize=30)
...
plt.ylabel('Number of orthogroups', fontsize=30)
